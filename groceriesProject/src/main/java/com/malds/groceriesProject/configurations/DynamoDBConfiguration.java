...
public class DynamoDBConfiguration {
        ...
        public DynamoDBMapper dynamoDBMapper() throws FileNotFoundException {
                ...
                /*
                 * List<List<String>> records = new ArrayList<>(); try (Scanner scanner = new
                 * Scanner(new File("../../../../Downloads/throw/product_mock.csv"));) {
        ...
        public AmazonDynamoDB amazonDynamoDB() {
                 ...
                 * 
                 * ListTablesResult table_list = createdClient.listTables(request); List<String>
                 * table_names = table_list.getTableNames();
                 * 
                 * if (table_names.size() > 0) { for (String cur_name : table_names) {
                 * System.out.format("* %s\n", cur_name); } } else {
                 * System.out.println("No tables found!"); System.exit(0); }
                 * 
                 * last_name = table_list.getLastEvaluatedTableName(); if (last_name == null) {
                 * more_tables = false; } }catch (AmazonServiceException e){
